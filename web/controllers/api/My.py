...
def myOrderList():
    ...
    if pay_order_list:
        ...
        quant_ids = selectFilterObj(pay_order_item_list, "quant_id")
...
def myOrderInfo():
    ...
    if pay_order_items:
        quant_ids = selectFilterObj(pay_order_items, " quant_id")
        quant_map = getDictFilterField( Quant,  Quant.id, "id",  quant_ids)
...
def myCommentAdd():
    ...
    pay_order_items = PayOrderItem.query.filter_by(pay_order_id=pay_order_info.id).all()
    quant_ids = selectFilterObj(pay_order_items, " quant_id")
    tmp_quant_ids_str = '_'.join(str(s) for s in quant_ids if s not in [None])
    model_comment = MemberComments()
    model_comment. quant_ids = "_%s_" % tmp_quant_ids_str
