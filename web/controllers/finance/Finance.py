    ...
    if pay_list:
        ...
        if pay_order_items_map:
            ...
            for item in pay_order_items_map:
                tmp_quant_ids = selectFilterObj(pay_order_items_map[item], "quant_id")
                tmp_quant_ids = {}.fromkeys(tmp_quant_ids).keys()
                quant_ids = quant_ids + list(tmp_quant_ids)

            #quant_ids里面会有重复的，要去重
            quant_mapping = getDictFilterField(Quant, Quant.id, "id", quant_ids)
        ...
        for item in pay_list:
            ...
            for tmp_order_item in tmp_order_items:
                tmp_quant_info = quant_mapping[tmp_order_item.quant_id]
                tmp_quants.append({
                    'name': tmp_quant_info.name,
                    'quantity': tmp_order_item.quantity
                })

            tmp_data['quants'] = tmp_quants
...
def info():
    ...
    if order_item_list:
        quant_map = getDictFilterField(Quant, Quant.id, "id", selectFilterObj(order_item_list, "quant_id"))
